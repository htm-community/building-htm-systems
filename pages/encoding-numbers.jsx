...
class EncodingNumbers extends React.Component {
  ...
  animateValueTo = (target) => {
    	...
    	this.animationHandle = setInterval(() => {
    		...
    		const current = Math.round(value + (diff * ratio))
  ...
  render() {
  	...
  	return (
          ...
          Given constant values for the input value range and output parameters, a complete scalar encoder.
  				...
  				<p>Notice how the range of the on bits in the encoding always encompass the currently selected value. As you move towards the min and max values, you might notice there is a problem with this representation. If you increase the resolution and move the value, you can clearly see the number of bits in the representation decreasing by half as you approach the min and max values (watch the figure above as you <a className='interactive' onClick={() => this.animateValueTo(this.state.min)}>click here</a>). Did you notice anything? <a>Click again</a> and pay attention to the size of the encoding. It changes as the value moves toward the edge, and that breaks one of our <a href="/encoders"> established earlier</a>. Principle #4 of encoders states:</p>
